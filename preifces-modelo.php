...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($_POST['simul-comando'] == 'act') {
    ...
    $simul_status = 1;
    $file_path = $banco . $filename;
    $doc_url = $banco . 'G' . $grado . 'S' . $orden3 . "." . $ext;

    try {

      if (!empty($_FILES['simul-pdf']['tmp_name'])) {

        if (file_exists($file_path)) {
          unlink($file_path);
        }

        move_uploaded_file($_FILES['simul-pdf']['tmp_name'], $file_path);
  ...
  if ($_POST['simul-comando'] == 'respuestas') {
    ...
    $hayClavesVacias = false;

    foreach ($respuestas as $key => $value) {
      if (trim($value) === '') {
        $hayClavesVacias = true;
        break;
      }
    }
    ...
    try {
      ...
      $alum_gradox = $result_id2x['gdo_id'];
  ...
  if ($_POST['simul-comando'] == 'respuestas2') {
    ...
    $desc = 'simul_materia_lenguaje';

    $simul_rev_array = array(
      'simul_materia_ingles' => 'simulr_ingles',
      'simul_materia_naturales' => 'simulr_naturales',
      'simul_materia_lenguaje' => 'simulr_lenguaje',
      'simul_materia_matematicas' => 'simulr_matematicas',
      'simul_materia_sociales' => 'simulr_sociales',
      'simul_materia_filosofia' => 'simulr_filosofia',
      'simul_materia_fisica' => 'simulr_fisica',
    );

    $result_mat_desc2 = $simul_rev_array[$result_mat_desc];
  ...
  if ($_POST['simul-comando'] == 'nuevo_plazo') {
    ...
    if ($type == 1) {
      ...
      try {
        ...
        $stmt->bind_param("iisss", $simul, $alum, $plazo, $status, $hoy);
        $stmt->execute();
        if ($stmt->insert_id) {
          $respuesta = array(
            'respuesta' => 'exito'
